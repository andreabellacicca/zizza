<script lang="ts">
    ...
    import {getUserWallets, updateUserWalletsById} from "$lib/apis/users";
    ...
    const submitHandler = async () => {
        ...
        const updatedWallet = await updateUserWalletsById(localStorage.token, $user.id, {near_pk: NearPk, zec_words: ZCashWords}).catch(
            (error) => {
                toast.error(`${error}`);
            }
        );
                    ...
                    <div class="justify-between w-full">
                        ...
                        <div class="flex mt-2">
                            {#if ZCashWords}
                                ...
                                <Tooltip content={$i18n.t('Create new NEAR Account')}>
                                    <button
                            ...
                            {:else}
                                ...
                                >
                                    ...
                                    {$i18n.t('Create new ZCash private key')}</button
                                >
                            {/if}
                        </div>
                    </div>
