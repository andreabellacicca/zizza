...
const updateChat = async () => {
	if ((selectedModels.indexOf("Supervise Request") > -1) &&
		(history.messages[history.currentId]?.done === true) &&
		(history.messages[history.currentId].content == "Revision started.")) {
		chat = await getChatById(localStorage.token, $chatId).catch(async (error) => {
			await goto('/');
			return null;
		});
		...
		if (chatContent) {
			history =
				(chatContent?.history ?? undefined) !== undefined
					? chatContent.history
					: convertMessagesToHistory(chatContent.messages);
			// serve per portare in fondo lo scroll
			// Se lo attivo, vuol dire che porta alla fine lo scroll
			// anche se un utente sta solo leggendo l'history
			//window.setTimeout(() => scrollToBottom(), 0);
		}
	}
}
