<script lang="ts">
	...
	const updateChatList = async () => {
		...
		if (search) {
			for(var i=0; i< $currentChatPage; i++){
				let new_chat = await getChatListBySearchText(localStorage.token, search, i)
				if (new_chat.length == 0) {
					currentChatPage.set(i-1)
					break
				}
				all_chats.push(...new_chat);
			}
		} else {
			for(var i=1; i< $currentChatPage; i++) {
				let new_chat = await getChatList(localStorage.token, i)
				if (new_chat.length == 0) {
					currentChatPage.set(i - 1)
					break
				}
